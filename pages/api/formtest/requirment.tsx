...
async function handler(request: NextApiRequest, response: NextApiResponse): Promise<void> {
  ...
  try {
    const responseAPI = await fetch(
      `https://app.tabidoo.cloud/api/v2/apps/${process.env.TABIDOO_APP_NAME_TEST}/tables/dotaz/data`,
      {
        method: "POST",
    ...
    response.status(500).send("Unexpected error on /api/form/requirment handler.");
    return;
  }
}
